...
TEST(NeuronTest, MembranePotential) {
    ...
    for(unsigned int i(481); i < (481 + REFRACTORY_TIME); ++i) {
        EXPECT_LT(fabs(simulation1.getNeuronV(0)[481]), EPSILON);
    }
    EXPECT_GT(fabs(simulation1.getNeuronV(0)[481 + REFRACTORY_TIME + 1]), EPSILON);

    //! Spike 2
    EXPECT_LT(fabs(simulation1.getNeuronV(0)[981] - 20), EPSILON);
    for(unsigned int i(982); i < (982 + REFRACTORY_TIME); ++i) {
        EXPECT_LT(fabs(simulation1.getNeuronV(0)[982]), EPSILON);
    }
    EXPECT_GT(fabs(simulation1.getNeuronV(0)[982 + REFRACTORY_TIME + 1]), EPSILON);
 ...
 */
TEST(CurrentTest, ValueStartStop) {
...
TEST(BufferTest, SizeAndErase) {
    ...
    for(unsigned int i(0); i < 481; ++i) {
        simulation.loop();
        EXPECT_EQ(buffer->size(), DELAY + 1);
    }
    //! Spike 1 occurs at time 480 - neuron resets at time 481

    //! we verify that neuron writes into buffer of other neuron
    EXPECT_LT(simulation.getNeuron(1)->getBuffer()->amplitude(480 + DELAY) - J_AMP, EPSILON);

    //! we then continue simulation - without exceding buffer size
    for(unsigned int i(480); i < 480 + DELAY; ++i) {
        simulation.loop();
        EXPECT_EQ(buffer->size(), DELAY + 1);
    }

    //! verifying that buffer erases transmitted spike
    EXPECT_LT(simulation.getNeuron(1)->getBuffer()->amplitude(480 + DELAY), EPSILON);
 ...
 *
 */
TEST(ConnectionTest, Transmittance) {
    ...
    EXPECT_GT(fabs(simulation.getNeuronV(0)[1481 + REFRACTORY_TIME + 1]), EPSILON);

	//! Spike 1 Transmission to neuron 1 - will spike to 1st transmission
    EXPECT_LT(fabs(simulation.getNeuronV(1)[1480 + DELAY] - 20), EPSILON);
    EXPECT_GT(fabs(simulation.getNeuronV(1)[1482 + DELAY + REFRACTORY_TIME + 1]), EPSILON);
    
    //! Spike Transmission to neuron 2 - small spike (0.1 mV)
    EXPECT_LT(fabs(simulation.getNeuronV(2)[1480 + 2 * DELAY] - 0.1), EPSILON);
